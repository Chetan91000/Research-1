...
Despite these developments, current LLM-assisted security tools face operational challenges:
...
### Research Objectives
1. **Construct a Privacy-Preserving Local Architecture:** Develop an interactive application security assistant powered exclusively by locally hosted LLMs, ensuring strict local loopback execution (`127.0.0.1`) to reduce external telemetry leakage.
...
4. **Establish Deterministic Replay Comparison:** Implement a closed-loop differential replay engine that evaluates re-test evidence streams against baseline findings to verify remediation status.
...
Sentinel Desktop is structured as a modular, closed-loop interactive application security assistant. The system comprises five core modules: Local AI Layer, Browser Interaction Module, Security Analysis Engine, Context Management Engine, and Replay Comparison & Evidence Collection Module.
...
The inert canary evaluation comprised 50 trials across four target conditions. Binary reflection detection achieved 100.0% accuracy ($TP=40, TN=10, FP=0, FN=0$). However, exact context classification achieved 70.0% accuracy due to limitations with the `ATTRIBUTE` context. Table III details the context-level results breakdown.
...
| :--- | :--- | :--- | ---: | ---: | ---: | ---: | ---: | ---: |
| **Exp 1** | Replay Comparison | `127.0.0.1:4567` (`/` vs `/fixed`) | 30 | 100.0% | 100.0% | 100.0% | 1.000 | 1.12 ms |
